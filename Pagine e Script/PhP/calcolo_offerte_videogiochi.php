<?php
	// LO SCRIPT PERMETTE DI TENERE TRACCIA DEL NUMERO DELLE PROPOSTE DI VENDITA CHE SI RIFERISCONO A DELLE OPERE VIDEOLUDICHE
	...
	for($i=0; $i<$offerte->length; $i++){
		...
		for($j=0; $j<$prodotti->length; $j++) {
			...
			if($prodotti->item($j)->getAttribute("id")==$offerta->getAttribute("idProdotto") && $prodotti->item($j)->getElementsByTagName("videogioco")->length!=0) {
				$num_offerte++;
			}
		}
	}
