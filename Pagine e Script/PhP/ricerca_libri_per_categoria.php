<?php
	// LO SCRIPT PERMETTE DI RICERCARE GLI ARTICOLI RELATIVI A DELLE OFFERTE E APPARTENENTI ALLA CATEGORIA SELEZIONATA
	...
	// 1) NESSUN ORDINAMENTO => IL VETTORE DI CUI SOPRA, AL CONTRARIO DELLE ALTRE VOCI, NON SARÀ ASSOCIATIVO E, DI CONSEGUENZA, IL CONTENUTO DI CIASCUNA DELLE CELLE CHE LO COMPONGONO SI RIFERÀ ALL'IDENTIFICATORE DELL'OFFERTA i-ESIMA, IL CUI ARTICOLO DI RIFERIMENTO APPARTIENE ALLA CATEGORIA SELEZIONATA IN PRECEDENZA
	...
	// N.B.: L'ORDINAMENTO A SECONDA DEL GENERE LETTERARIO È STATO IMPLEMENTATO ISOLANDO LE OPERE CARTACEE IN BASE ALLA LORO CATEGORIA DI APPARTENENZA, PERTANTO TRAMITE LA CREAZIONE DELLA PAGINA IN CUI VIENE RICHIAMATO TALE SCRIPT
	
	$libri_per_categoria=array();
	...
	for($i=0; $i<$offerte->length; $i++)
	{
		...
		for($j=0; $j<$prodotti->length; $j++)
		{
			...
			{
				for($k=0; $k<$prodotto->getElementsByTagName("categoria")->length; $k++)
				{
					if($categoria->getAttribute("id")==$prodotto->getElementsByTagName("categoria")->item($k)->getAttribute("idCategoria"))
					{
						...
						if(isset($_GET["ordinamento"]))
						{
							...
							if($_GET["ordinamento"]=="prezzoCrescente" || $_GET["ordinamento"]=="prezzoDecrescente")
							{
								...
								if(!$offerta->getElementsByTagName("scontoATempo")->length)
									$libri_per_categoria=array_merge($libri_per_categoria, array("'".$offerta->getAttribute("id")."'" => $offerta->getElementsByTagName("prezzoContabile")->item(0)->textContent));
								else
									$libri_per_categoria=array_merge($libri_per_categoria, array("'".$offerta->getAttribute("id")."'" => number_format(floatval($offerta->firstChild->textContent) - (floatval($offerta->firstChild->textContent)*(floatval($offerta->getElementsByTagName("scontoATempo")->item(0)->getAttribute("percentuale"))/100)),2,".","")));
							}
							else {
								if($_GET["ordinamento"]=="nomeA-Z" || $_GET["ordinamento"]=="nomeZ-A")
								{
									$libri_per_categoria=array_merge($libri_per_categoria, array("'".$offerta->getAttribute("id")."'" => $prodotto->firstChild->textContent));
								}
								else {
									if($_GET["ordinamento"]=="annoDiUscita")
									{
										$libri_per_categoria=array_merge($libri_per_categoria, array("'".$offerta->getAttribute("id")."'" => intval($prodotto->getElementsByTagName("annoUscita")->item(0)->textContent)));
									}
									// QUALORA SIA STATO RICHIESTO UN ORDINAMENTO NON PREVISTO DALLA PAGINA DEL SITO, VERRÀ INTERPRETATO COME SE FOSSE QUELLO DI DEFAULT, OVVERO NESSUNO 
										...
										array_push($libri_per_categoria, $offerta->getAttribute("id"));
									}
								}
							}
						}
