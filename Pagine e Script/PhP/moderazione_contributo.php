<?php
	// LO SCRIPT PERMETTE DI MODERARE IL CONTRIBUTO (RECENSIONE, DISCUSSIONE O INTERVENTO) SELEZIONATO ALL'INTERNO DELLA SCHEDA RELATIVA AD UNA CERTA PROPOSTA DI VENDITA
	...
	// LO STESSO RAGIONAMENTO PUÒ ESSERE ESTESO ANCHE PER L'ENTITÀ DA SEGNALARE. IN OGNI CASO, AL FINE DI OPERARE CORRETTAMENTE SU UN CERTO INTERVENTO, SARÀ COMUNQUE NECESSARIO FARE RIFERIMENTO ALLA DISCUSSIONE A CUI APPARTIENE
	...
	// DATA LA LORO SOMIGLIANZA, IL MECCANISMO APPLICATO IN PRECEDENZA PER LE OFFERTE POTRÀ ESSERE ESTESO ANCHE PER I SINGOLI ELEMENTI DA MODERARE
	...
	// PRIMA DI PROCEDERE CON LE VARIE OPERAZIONI DI INTERESSE, SARÀ NECESSARIO IMPEDIRE LA MODERAZIONE DI TUTTI QUEI CONTRIBUTI CHE SI RIFERISCONO A SOGGETTI INESISTENTI O AI GESTORI O ALL'AMMINISTRATORE DEL SITO
	if(isset($_GET["id_Recensione"])) {
	...
	if(isset($_GET["id_Recensione"])) {
		$recensione->setAttribute("moderata", "Si");
		...
		if(isset($_GET["id_Discussione"])) {
			if(!isset($_GET["id_Intervento"])) {
				$discussione->setAttribute("moderata", "Si");
				...
				$intervento->setAttribute("moderato", "Si");
